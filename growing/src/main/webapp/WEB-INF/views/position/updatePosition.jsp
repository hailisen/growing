...
<div id="add-dept">
    <div id="title">
        <label id="title-text">修改职称</label>
        ...
        <form action="${pageContext.request.contextPath}/position/addPosition" id="add-from" method="post">
            <table id="add-table">
                <tr>
                    ...
                    <td class="right-td">
                        <input class="input-text layui-input" type="text" name="positionName" id="positionName" value="${position.positionName}"  placeholder="请输入名称">
                        <input value="${position.id}" name="id" type="hidden">
                    </td>
                </tr>
                <tr>
    ...
    layui.use(['laydate','layer','upload'], function() {
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                for(var i=0;i<data.data.length;i++){
                    if(data.data[i].id == ${cateId}){
                        value+="<option value='"+data.data[i].id+"' selected>"+data.data[i].cateName+"</option>"
                    }else{
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                for(var i = 0;i<data.data.length;i++){
                    if(data.data[i].id == ${position.categoryId}){
                        value+="<option value='"+data.data[i].id+"' data-time='"+data.data[i].timeLimit+"' selected>"+data.data[i].level+"</option>"
                        $("#timeLimit").val(data.data[i].timeLimit);
                    }else{
