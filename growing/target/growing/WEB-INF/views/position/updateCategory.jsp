...
<head>
    ...
    <title>修改职称类别</title>
    <meta name="description" content="修改职称类别页面">
...
<div id="bigDiv">
    ...
    <div id="add-dept">
        <div id="title">
            <label id="title-text">修改职称类别</label>
            ...
            <form action="${pageContext.request.contextPath}/position/updateCategory" id="add-from" method="post">
                <table id="add-table">
                    <tr>
                        ...
                        <td class="right-td">
                            <input class="input-text layui-input" type="text" name="cateName" id="cateName" value="${category.cateName}"  placeholder="请输入名称">
                    ...
                    <tr>
                        ...
                        <td class="right-td">
                            <textarea placeholder="请输入描述" style="margin-left: 36px; margin-top: 6px;width: 328px;" id="cateNote" class="layui-textarea" name="note">${category.id}</textarea>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <c:forEach var="cateLevel" items="${requestScope.cateLevels}">
                        <tr data-cateLevel="1">
                            <c:choose>
                                <c:when test="${cateLevel.timeLimit==1}">
                                    <td>一级职称</td>
                                </c:when>
                                <c:when test="${cateLevel.timeLimit==2}">
                                    <td>二级职称</td>
                                </c:when>
                                <c:when test="${cateLevel.timeLimit==3}">
                                    <td>三级职称</td>
                                </c:when>
                                <c:when test="${cateLevel.timeLimit==4}">
                                    <td>四级职称</td>
                                </c:when>
                                <c:when test="${cateLevel.timeLimit==5}">
                                    <td>五级职称</td>
                                </c:when>
                            </c:choose>
                            <td>
                                <input class="input-text layui-input" type="text" name="levelName" class="levelName" value="${cateLevel.levelName}" style="display: inline-block;width: 150px !important;" placeholder="请输入名称">
                                <span style="display: inline;margin-left: 38px;">晋级年限</span>
                                <input class="input-text layui-input" onKeyUp="value=value.replace(/[^\d]/g,'')" type="text" name="timeLimit" class="timeLimit" value="${cateLevel.timeLimit}" style="display: inline-block;width: 42px !important;margin-left: 15px;">
                                &nbsp; 年</td>
                        </tr>
                    </c:forEach>
                    <tr>
...
<script>
    layui.use(['laydate','layer','upload'], function() {
        ...
        $("#levelNum").val(${category.levelNum})
        //返回主页
        $(".jexit").click(function(){
            $("#bigDiv").load("toCategory")
